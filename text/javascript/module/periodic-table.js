...
export let CSS = {
    ...
    init:function () {
        ...
        for ( let i = 0; i < this.table.length; i += 5 ) {// table
            const element = document.createElement( 'div' );
                element.className = 'element';
                element.style.backgroundColor = 'rgba(0,127,127,' + ( Math.random() * 0.5 + 0.25 ) + ')';
            const number = document.createElement( 'div' );
                number.className = 'number';
                number.textContent = ( i / 5 ) + 1; element.appendChild( number );
            const symbol = document.createElement( 'div' );
                symbol.className = 'symbol';symbol.textContent = this.table[ i ]; element.appendChild( symbol );
            const details = document.createElement( 'div' );
                details.className = 'details';
                details.innerHTML = this.table[ i + 1 ] + '<br>' + this.table[ i + 2 ];element.appendChild( details );
        ...
        const vector = new THREE.Vector3();// sphere
        ...
        for ( let i = 0, l = this.objects.length; i < l; i ++ ) {// helix
        ...
        for ( let i = 0; i < this.objects.length; i ++ ) {// grid
            const object = new THREE.Object3D();
    ...
    transform:function( targets, duration ) {
        ...
        for ( let i = 0; i < this.objects.length; i ++ ) {
            ...
            const target = this.targets[Object.keys(i)];
...
// export { init,table,targets, transform,animate }