...
var addInitialData = function(){
    ...
    while(this.data.length > 0 && this.firstRunTime + (next = this.data.pop()).when < Date.now()){this.addPin(next.lat, next.lng, next.label);}
    if(this.firstRunTime + next.when >= Date.now()){this.data.push(next);}};
var createParticles = function(){
    if(this.hexGrid){this.scene.remove(this.hexGrid);}
    ...
    var pointFragmentShader = [
        ...
        "}"].join("\n");
    var pointAttributes = {lng: {type: 'f', value: null}};
    this.pointUniforms = {currentTime: { type: 'f', value: 0.0}}
    ...
    var myColors = [];
    for(var i = 0; i< baseColorSet.length; i++){myColors.push(baseColorSet[i].shade(Math.random()/3.0));}
    /* break geometry into chunks of 21,845 triangles (3 unique vertices per triangle)
        for indices to fit into 16 bit integer number floor(2^16 / 3) = 21845 */
    var chunkSize = 21845;
    var indices = geometry.attributes.index.array;
    for ( var i = 0; i < indices.length; i ++ ) {indices[ i ] = i % ( 3 * chunkSize );}
    ...
    var addTriangle = function(k, ax, ay, az, bx, by, bz, cx, cy, cz, lat, lng, color){
        ...
        lng_values[p] = lng; lng_values[p+1] = lng; lng_values[p+2] = lng;
    ...
    for ( var i = 0; i < offsets; i ++ ) {
        var offset = { start: i * chunkSize * 3, index: i * chunkSize * 3, count: Math.min( triangles - ( i * chunkSize ), chunkSize ) * 3};
...
var createIntroLines = function(){
    var sPoint;
    var introLinesMaterial = new THREE.LineBasicMaterial({ color: this.introLinesColor, transparent: true, linewidth: 2, opacity: .5});
...
function Globe(width, height, opts){/* globe constructor */
